...
def compute_score(sandbox_fusion_url, concurrent_semaphore, completion, test_cases, continuous=False, timeout=30):
    """
...
def compute_score_old(sandbox_fusion_url, concurrent_semaphore, completion, test_cases, continuous=False, timeout=30):
    """
    Computes the code score using the remote sandbox API.
    ...
    try:
        ...
        # Assume check_correctness returns (results_list, metadata_list)
        # results_list contains True, False, or error codes (-1, -2, -3, etc.)
        res_list, metadata_list = check_correctness(sandbox_fusion_url=sandbox_fusion_url, in_outs=test_cases, generation=solution, timeout=timeout, concurrent_semaphore=concurrent_semaphore, language=language)
